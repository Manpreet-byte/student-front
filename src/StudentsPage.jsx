...
export default function StudentsPage() {
	...
	const loadFeedback = async () => {
		...
		try {
			...
			data.forEach(item => {
				...
				studentMap[item.studentName].totalRating += item.rating;
			});
	...
	const handleStudentNameClick = (student) => {
		...
		setQuickAddForm({ rating: 5, comment: '' });
	...
	// Calculate score out of 100 for each house (average rating × 20)
	const calculateHouseScore = (houseStudents) => {
		if (houseStudents.length === 0) return 0;
		const avgRating = houseStudents.reduce((sum, s) => sum + parseFloat(s.avgRating), 0) / houseStudents.length;
		return (avgRating * 20).toFixed(1);
	};
